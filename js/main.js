...
window.BrawlerImages = {
    menu: null,   // large (400px) for brawler stand
    icon: null,   // small (80px) for profile icon and grid
...
function loadBrawlerImages() {
    return new Promise((resolve) => {
        ...
        imgMenu.onload = () => { window.BrawlerImages.menu = imgMenu; checkAll(); };
        imgMenu.onerror = checkAll;
        imgMenu.src = 'images/mysteria_menu.png';

        const imgIcon = new Image();
        imgIcon.onload = () => { window.BrawlerImages.icon = imgIcon; checkAll(); };
        imgIcon.onerror = checkAll;
        imgIcon.src = 'images/mysteria_icon.png';

        const imgGame = new Image();
        imgGame.onload = () => { window.BrawlerImages.game = imgGame; checkAll(); };
...
async function claimDailyReward() {
    ...
    const { error } = await window.sb
        .from('profiles')
        .update({ coins: playerState.coins, daily_claimed: true, last_daily: now })
        .eq('user_id', window.currentUser.id);
...
function updateBrawlerMenu() {
    document.getElementById('brawler-stand').innerHTML = createBrawlerSVG(state.currentBrawler, 'xlarge');
